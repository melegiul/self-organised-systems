...
public class AntColony<T extends Comparable<T>> implements Comparator<T[]>{
    ...
    private static Double minPheromoneGlobal = Double.MAX_VALUE;
    private static Double maxPheromoneGlobal = Double.MIN_VALUE;
...
//        TreeSet<Integer[]> routeSet = new TreeSet<>();
    ...
    private void computePheromoneEntropy(int granularity) {
        ...
        if(maxPheromoneGlobal < maxPheromone){
            maxPheromoneGlobal = maxPheromone;
        }
        if(minPheromoneGlobal > minPheromone){
            minPheromoneGlobal = minPheromone;
        }
        minPheromone = 7.055079108655364e-15;
        maxPheromone = 8.9e-5;
        ...
        for (int i=0; i<granularity; i++) {
            if (i==0)
                ...
                quantification[i] = minPheromone;
            else
                quantification[i] = quantification[i-1] * 0.90;
        }
        ...
        for (int i=0; i<numberCities; i++) {
            for (int j=i+1; j<numberCities; j++) {
                ...
                while(pheromone < quantification[k] && k < granularity-1){
                    k++;
                }
...
//        }
    ...
    private void localUpdate() {
        for (int k=0; k<numberAnts; k++) {
            ...
            int nextCity = tour[k].get(tourLength - 1);
            int predecessorCity = tour[k].get(tourLength - 2);
